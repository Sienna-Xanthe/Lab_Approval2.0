...
//       $list_state=0;
    ...
    //    $form=Form::wzh_formdelete($form_id);
        $equipment=Equipment::wzh_equipmentdelete($form_id);
        $list=Lists::wzh_listdelete($form_id);

     //   $res['form']=$form;
        $res['$equipment']=$equipment;
        $res['$list']=$list;
        return $res?   //判断
            json_success("撤销成功",$res,200):
    ...
    public function borrow_delete(BorrowDelete $request)
    {
        $form_id = $request['form_id'];    //获取form_id编号
     //   $form=Form::wzh_formdelete($form_id);
        $equipment=Borrow::wzh_borrowdelete($form_id);
    ...
    public function borrow_look(BorrowLook $request)
    {
        ...
        $equipment=Borrow::wzh_borrowlook($form_id);
        return $equipment?   //判断
            json_success("查询成功",$equipment,200):
            json_fail("查询失败",null,100);
    }
...
//        $list_attachment=$request['list_attachment'];
    ...
    //    $form=Form::wzh_formdelete($form_id);
        $open=Open::wzh_opendelete($form_id);
        $applicant=Applicant::wzh_applicantdelete($form_id);
    //    $res['$form']=$form;
        $res['$open']=$open;
    ...
    public function run_add(RunAdd $request)
    {
       ...
       // $form=Form::wzh_formadd($form_id,$form_name_id,$login_id,$form_state1,$form_state2,$form_state3,$form_state4);
        $run=\App\Models\Run::wzh_runadd($run_id,$run_week,$run_time,$run_class,$run_name,$run_number,$run_cname,$run_type,$run_teacher,$run_situation,$run_note);
      //  $res['form']=$form;
      //  $res['run']=$run;
        return $run?   //判断
    ...
    /** * 实验室运行记录表查看数据
     * @param RunLook $request
     * @return \Illuminate\Http\JsonResponse
     * @throws \Exception
     */
    public function run_look(RunLook $request)
    {
        $run_id = $request['run_id'];    //获取form_id编号
        $res=\App\Models\Run::wzh_runlook($run_id);
        return $res?   //判断
            json_success("查询成功",$res,200):
            json_fail("查询失败",null,100);
    }
    ...
    public function find(Find $request)
    {
     ...
     $state=$request['state'];
     $res=Form::find1($form_name_id,$state);
